...
const Home: React.FC = () => {
  ...
  const topChatbots = sortedChatbots.slice(0, 4);
            ...
            {Object.entries(CATEGORIES).map(
              ([category, { color, description }]) => (
                <Link key={category} to={`/chatbots?category=${category}`}>
                  <Card
                    className={`bg-${color}/10 hover:bg-${color}/20 transition-colors cursor-pointer h-full`}
                  >
                    <CardContent className="p-4 flex flex-col items-center justify-center text-center h-full">
                      <div
                        className={`w-12 h-12 rounded-full bg-${color} mb-3 flex items-center justify-center text-white`}
                      >
                        {category.charAt(0)}
                      </div>
