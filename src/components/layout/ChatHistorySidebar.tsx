...
export const ChatHistorySidebar: React.FC<ChatHistorySidebarProps> = ({
  ...
  // Updated sorting function for chat history - sort by created_at descending
  const sortChatsByDate = useCallback(
    (chats: Chat[]): Chat[] => {
      return [...chats].sort((a, b) => {
        // Parse created_at dates from both chats
        const aCreatedAt = parseDate(a.createdAt);
        const bCreatedAt = parseDate(b.createdAt);

        // Sort in descending order (most recent created_at first)
  ...
  const loadChatHistory = useCallback(async () => {
    ...
    console.log(
      "Loading chat history for chatbot:",
      chatbot.name,
      "Category:",
      chatbot.category
    );
    ...
    try {
      ...
      if (Array.isArray(history)) {
        // Apply enhanced sorting by created_at
    ...
    async (e: React.MouseEvent, chatId: string) => {
      ...
      try {
        ...
        if (success) {
          ...
          setChatHistory((prev) =>
            prev.filter((chat) => chat.chatId !== chatId && chat.id !== chatId)
          );
  ...
  useEffect(() => {
    console.log(
      "ChatHistorySidebar useEffect triggered, chatbot:",
      chatbot?.name
    );
    loadChatHistory();
  }, [loadChatHistory]);

  // Auto-refresh chat history when typing state changes (indicates message sent/received)
  useEffect(() => {
    // Only reload if we were typing and now we're not (message completed)
    // Also check if current chat has a valid chatId (not a temporary chat)
    if (!isTyping && currentChat?.chatId) {
  ...
  }, [isTyping, currentChat?.chatId, loadChatHistory]);

  // Also reload when coming back to the page
  useEffect(() => {
    const timer = setTimeout(() => {
  ...
  const formatChatDate = useCallback(
    (chat: Chat) => {
      try {
        // Use createdAt for display since we're sorting by it
        ...
        return format(createdDate, "MMM d, h:mm a");
      } catch (error) {
